...
namespace wali
{
  ...
  {  
    ...
    class Label : public Printable
    {
      public:
        typedef Key Sym;
        typedef std::set<Sym>::const_iterator iterator;
       ...
       * 
       */
      const std::set<Sym> & getSymbolsIn( ) const;
       ...
       * 
       */
      const std::set<Sym> & getSymbolsNotIn( ) const;
    ...
    bool Label::isWild( ) const
    {
      ...
      else
      {
        ...
        //for( iterator it = symbolPool.begin(); it != symbolPool.end(); it++ )
        //{
    ...
    void Label::addAll( Label lbl )
    {
      if( lbl.neg )
      {
        ...
        //for( iterator it = symbolPool.begin(); it != symbolPool.end(); it++ )
        //{
        //  if( symSet.containsSymbol(*it) )
        //    addSymbol(*it);
        //}
      }
      else
      {
        for( iterator it = lbl.syms.begin(); it != lbl.syms.end(); it++ )
        {
          addSymbol(*it);
        }
      }
     ...
     *
     */
    void Label::removeAll( Label lbl )
    {
      if( lbl.neg )
      {        
        //Note: we cannot directly add symbols from syms because syms only 
     ...
     *
     */
    std::ostream & Label::print( std::ostream & o ) const
    {
      if( isWild() )
        o << "wild";
      else if( isAbsent() )
      ...
      else
      {
        ...
        bool first = true;
        for( iterator it = syms.begin(); it != syms.end(); it++ )
        {
          if( !first )
          {
            o << ", "; 
     ...
     * 
     */
    const std::set<typename Label::Sym> & Label::getSymbolsIn( ) const
    {
     
      if( neg )
      { 
        std::set<Sym> * symsIn = new std::set<Sym>;
        // TODO: fix this!
        //for( iterator it = symbolPool.begin(); it != symbolPool.end(); it++ )
        //{
     ...
     * 
     */
    const std::set<typename Label::Sym> & Label::getSymbolsNotIn( ) const
    {      
      if( neg )
      {
        return syms;
      ...
      {
        std::set<Sym> * symsNotIn = new std::set<Sym>();
        // TODO: fix this!
        //for( iterator it = symbolPool.begin(); it != symbolPool.end(); it++ )
        //{
        //  if(! containsSymbol(*it) )
     ...
     *
     */
    size_t Label::sizeSymbolsIn( ) const
    {
      // TODO: fix this!
      //if( neg )
      //  return (symbolPool.size() - syms.size());
      //else
        return syms.size();
    }
     ...
     *
     */
    size_t Label::sizeSymbolsNotIn( ) const
    {
      // TODO: fix this!
      //if( neg )
        return syms.size();
    ...
    bool SymbolSet::operator==( const SymbolSet & other ) const
    {
      ...
      for( iterator it = other.symbols.begin(); it != other.symbols.end(); it++ )
      {
        if (! isSymbol(*it) )
          return false;
      }
     ...
     */
    inline
    SymbolSet::iterator SymbolSet::beginSymbols( ) const
    {
     ...
     */
    inline
    SymbolSet::iterator SymbolSet::endSymbols( ) const
    {
      return symbols.end();
    }    
