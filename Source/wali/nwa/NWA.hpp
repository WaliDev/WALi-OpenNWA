  ...
  namespace nwa
  {
    ...
    class NWA : public Printable
    {
      public:
        ...
        //typedef std::set< StatePair > StatePairSet; TODO this belongs to a broken bit
        //typedef std::map< StatePairSet,St* > StateMap; TODO this belongs to a broken bit
       ...
       *
       * This method sets the client information associated with the given
       ...
       *
       * This method provides access to the state with the given name via
       * a pointer.
       *
      ...
      /**
       * 
       * @brief obtains the names of all states that are predecessors of the given 
       *        state
       *
       * This method returns the names of all states that are predecessors of the 
       * given state.
       *
       ...
       *
       * This method assigns to the duplicate state all the state properties
       * of the original state and duplicates all transitions of the original
       * state for the duplicate state.  Further, any self-loop transitions
       * that the original state are not only duplicated as self-loop transitions
       * for the duplicate state, but the duplicate state also has transitions
       * to and from the original state for each such transition.
       *
       * @param - orig: the name of the original state, i.e. the state to duplicate
       * @param - dup: the name of the duplicate state
       * @return a pointer to the newly created duplicate state
       ...
       * associated with it.
       *
       * @return set of states associated with the NWA
       *
       */
       ...
       *
       * This method tests whether the given state is in the 
       * state set of this NWA.
       *
       ...
       *
       * This method adds the given state to the state set for 
       * this NWA.  If the state already exists in the NWA, false is 
       ...
       *
       * This method checks for the given state in the set of initial states, 
       * the set of final states, and the set of states in this NWA.
       * It then removes the state from any set that contained it.
       ...
       *
       * This method provides access to the names of all states in the initial 
       * state set of this NWA.  Note: An NWA can have an unbounded number of 
       * inital states associated with it.
       *
       * @return set of names for all inital states associated with 
       *          the NWA
       *
       */
       ...
       *
       * This method tests whether the given state is in the initial
       ...
       *
       * This method checks for the given state in the state set of this
       * NWA.  If the state does not exist, it is added to the state set 
       * of the NWA.  The given state is then added to the set of initial 
       * states for the NWA.  If the state already exists in the intial 
       * state set of the NWA, false is returned.  Otherwise, true is returned.
       *
       * @param - state: the state to add to initial state set
       * @return false if the state already exists in the initial state set
       *          of the NWA, true otherwise
       *
       */
       ...
       *
       * This method checks for the given state in the initial state set of this 
       * NWA.  If the state exists, it is removed from the initial state set 
       * (but not from the set of all states of the NWA).  Note: This method 
       * does not remove any transitions from the NWA.
       *
       ...
       * NWA.  Note: This method does not remove any transitions from the NWA.
       *
       */
       ...
       *
       * This method provides access to the names of all states in the final 
       * state set of this NWA.  Note: An NWA can have an unbounded number of 
       ...
       *
       * This method checks for the given state in the state set of the NWA.  
       * If the state does not exist, it is added to the state set of the 
       * NWA.  The given state is then added to the set of final states for the 
       ...
       *
       * This method checks for the given state in the final
       ...
       *
       * This method removes all symbols associated with the NWA.  It also
       * removes all transitions associated with the NWA as there can 
       * be no transitions without at least one symbol.
       *
       */
      void clearSymbols( );
       ...
       *
       * This method checks for a transition (of any kind) with the given endpoints.
       * If such a transition exists true is returned and the symbol labeling that 
       * edge is placed in the reference parameter 'sym'.
       *
...
//TODO sync comments
     ...
     * TODO: write comments
     */
    //void intersectClientInfoPred( Return return1, Return return2, StName result );
    ...
    bool NWA<St,StName,Sym>::removeState( St * state )
    {
      ...
      if( states.removeState(state) )
      {
        ...
        trans->removeCallTransWith(state);
        trans->removeInternalTransWith(state);
        trans->removeReturnTransWith(state);
...
//TODO sync comments
     ...
     */
    //template <typename St,typename StName,typename Sym>
    //void NWA<St,StName,Sym>::intersectClientInfoPred( Return return1, Return return2, StName result ) 
    //{ 
      //Note: When overriding this method your metric must combine any client information associated
      // with the target states of the two transitions and set the client information of result
      // to that value.
...
*/
...
#else //ifdef KEY
    template<typename Client = ClientInfo,typename Sym = Symbol<> >
    class NWA : public Printable
    {
      public:
        ...
        typedef Triple<St,Sym,St> Call;       //TODO: change the St to Key?
        typedef Triple<St,Sym,St> Internal;   //TODO: change the St to Key?
        typedef Quad<St,St,Sym,St> Return;    //TODO: change the St to Key?
        typedef TransSet<St,Sym,Call,Internal,Return> Trans;
       ...
       * TODO write comments
       */
      ClientInfoRefPtr getClientInfo( wali::Key name ) const;

      /**
       * TODO write comments
       */
      void setClientInfo( wali::Key name, const ClientInfoRefPtr c );
       ...
       *
       */
      St* getState( const Key & name ) const;
       ...
       *
       */    
      std::set<Key> getPredecessorNames( const Key & name ) const;
       ...
       *
       */
      St* duplicateState( const Key & orig, const Key & dup );
       ...
       *
       */
      std::set<Key> getFinalStates( ) const;
       ...
       */
      bool getSymbol( Key from, Key to, Sym & sym );

      /**
       * TODO: write comments
       */
      bool findTrans( Key from, const Sym & sym, Key to) const;
       ...
       * TODO: write comments
       */
      bool addCallTrans( Key from, const Sym & sym, Key to );
       ...
       * TODO: write comments
       */
      bool removeCallTrans( Key from, const Sym & sym, Key to );
       ...
       * TODO: write comments
       */
      bool addInternalTrans( Key from, const Sym & sym, Key to );
       ...
       * TODO: write comments
       */
      bool removeInternalTrans( Key from, const Sym & sym, Key to );
       ...
       * TODO: write comments
       */
      std::set<Key> getReturnSites( Key callSite ) const;

      /**
       * TODO: write comments
       */
      bool addReturnTrans( Key from, Key pred, const Sym & sym, Key to );
       ...
       * TODO: write comments
       */
      bool removeReturnTrans( Key from, const Sym & sym, Key to );
       ...
       * TODO: write comments
       */
      bool removeReturnTrans( Key from, Key pred, const Sym & sym, Key to );
       ...
       * TODO: write comments
       */
      virtual bool nodeIntersect( const St * node1, const St * node2, St * & result );
       ...
       *
       */
      //static nwa::NWA PDStoNWA( wpds::WPDS pds ); //FIX
    ...
    //Constructors and Destructor
    template<typename Client,typename Sym>
    NWA<Client,Sym>::NWA( )
    {
      states = States();      
    ...
    template<typename Client,typename Sym>
    NWA<Client,Sym>::NWA( const NWA<Client,Sym> & other )
    {
      //Copy data over from 'other'
      states = other.states;
    ...
    template<typename Client,typename Sym>
    NWA<Client,Sym> & NWA<Client,Sym>::operator=( const NWA<Client,Sym> & other )
    {
      if (this == &other)     
        return *this;
     ...
     */
    template<typename Client,typename Sym>
    typename NWA<Client,Sym>::ClientInfoRefPtr NWA<Client,Sym>::getClientInfo( wali::Key name ) const
    {
      St * st = getState(name);
      return st->getClientInfo();
    }
     ...
     */
    template<typename Client,typename Sym>
    void NWA<Client,Sym>::setClientInfo( wali::Key name, const ClientInfoRefPtr c )
    {
      St * st = getState(name);
      st->setClientInfo(c);
    }
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::isAbsentAccept( ) const
    {
      return absentAcceptance;
    }
     ...
     */
    template<typename Client,typename Sym>
    void NWA<Client,Sym>::acceptAbsent( )
    {
      absentAcceptance = true;
    }
     ...
     */
    template<typename Client,typename Sym>
    void NWA<Client,Sym>::rejectAbsent( )
    {
      absentAcceptance = false;
    }
     ...
     */
    template<typename Client,typename Sym>
    typename NWA<Client,Sym>::St* NWA<Client,Sym>::getState( const Key & name ) const
    {
      return states.getState(name);
    }
     ...
     */
    template<typename Client,typename Sym>
    std::set<typename NWA<Client,Sym>::St*> NWA<Client,Sym>::getStates( ) const
    {
      std::set<St*> result;
      std::set<St> myStates;
      for( std::set<St>::iterator it = myStates.begin(); it != myStates.end(); it++ )
      {
        result.insert(&(*it));
      }
      return result;
     ...
     */
    template<typename Client,typename Sym>
    std::set<Key> NWA<Client,Sym>::getStateNames( ) const
    {
      return states.getStateNames();
    }
     ...
     */    
    template<typename Client,typename Sym>
    std::set<Key> NWA<Client,Sym>::getPredecessorNames( const Key & name ) const
    {
      St* state = getState(name);
     ...
     */
    template<typename Client,typename Sym>
    typename NWA<Client,Sym>::St * NWA<Client,Sym>::duplicateState( const Key & orig, const Key & dup )
    {
      St * origSt = getState(orig);
      St * dupSt = new St(dup);
    ...
    /* 
    template<typename Client,typename Sym>
    const typename NWA<Client,Sym>::States & NWA<Client,Sym>::getStates( )
    {
      return states.getStates();
    }*/
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::isState( const St * state ) const
    {
      return states.isState(*state);
    }    
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::addState( St * state )
    {
      return states.addState(state);
    }
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::addState( Key name )
    {
      St * st = new St(name);
      return addState(st);
    }
    //template<typename Client,typename Sym>
    //bool NWA<Client,Sym>::addState( )
    //{
    //  St * st = new St();
    //  return addState(st);
    //}
     ...
     */
    template<typename Client,typename Sym>
    void NWA<Client,Sym>::addAllStates( States addStateSet )
    {
      states.addAllStates(addStateSet);
    }    
     ...
     */
    template<typename Client,typename Sym>
    size_t NWA<Client,Sym>::sizeStates( ) const
    {
      return states.sizeStates();
    }
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::removeState( St * state )
    {
      //Remove transitions associated with the state that was removed.
      if( states.removeState(*state) )
      {
     ...
     */
    template<typename Client,typename Sym>
    void NWA<Client,Sym>::clearStates( )
    {      
      states.clearStates();
     ...
     */
    template<typename Client,typename Sym>
    typename NWA<Client,Sym>::stateIterator NWA<Client,Sym>::beginStates( ) const
    {
      return states.beginStates();
    }
     ...
     */
    template<typename Client,typename Sym>
    typename NWA<Client,Sym>::stateIterator NWA<Client,Sym>::endStates( ) const
    {
      return states.endStates();
    }    
     ...
     */
    template<typename Client,typename Sym>
    Key NWA<Client,Sym>::getInitialState( ) const
    {
      std::set<Key> sts = states.getInitialStateNames();
      assert(sts.size()==1); // Assume unique initial state
      return *(sts.begin());
    }
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::isInitialState( const St * state ) const
    {
      return states.isInitialState(*state); 
    }    
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::addInitialState( St * state )
    {
      return states.addInitialState(state);
    }
     ...
     */ 
    template<typename Client,typename Sym>
    void NWA<Client,Sym>::addAllInitialStates( States addStateSet )
    {
      states.addAllInitialStates(addStateSet);
    }
     ...
     */
    template<typename Client,typename Sym>
    size_t NWA<Client,Sym>::sizeInitialStates( ) const
    {
      return states.sizeInitialStates();
    }
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::removeInitialState( St * state ) 
    {
      return states.removeInitialState(*state);
    }    
     ...
     */
    template<typename Client,typename Sym>
    void NWA<Client,Sym>::clearInitialStates( )
    {
      states.clearInitialStates();
    }
     ...
     */
    template<typename Client,typename Sym>
    typename NWA<Client,Sym>::stateIterator NWA<Client,Sym>::beginInitialStates( ) const
    {
      return states.beginInitialStates();
    }
     ...
     */
    template<typename Client,typename Sym>
    typename NWA<Client,Sym>::stateIterator NWA<Client,Sym>::endInitialStates( ) const
    {
      return states.endInitialStates();
    }    
    ...
    template<typename Client,typename Sym>
    std::set<Key> NWA<Client,Sym>::getFinalStates( ) const
    {
      return states.getFinalStateNames();
    }
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::isFinalState( const St * state ) const
    {
      return states.isFinalState(*state);
    }    
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::addFinalState( St * state )
    {
      return states.addFinalState(state);
    }
     ...
     */
    template<typename Client,typename Sym>
    void NWA<Client,Sym>::addAllFinalStates( States addStateSet, bool accept )
    {
      if( accept )
        absentAcceptance = true;
     ...
     */
    template<typename Client,typename Sym>
    size_t NWA<Client,Sym>::sizeFinalStates( ) const
    {
      if( absentAcceptance )
        return states.sizeFinalStates() + 1;
      else
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::removeFinalState( St * state )
    {
      return states.removeFinalState(*state);
    }    
     ...
     */
    template<typename Client,typename Sym>
    void NWA<Client,Sym>::clearFinalStates( )
    {
      states.clearFinalStates();
     ...
     */
    template<typename Client,typename Sym>
    typename NWA<Client,Sym>::stateIterator NWA<Client,Sym>::beginFinalStates( ) const
    {
      return states.beginFinalStates();
    }
     ...
     */    
    template<typename Client,typename Sym>
    typename NWA<Client,Sym>::stateIterator NWA<Client,Sym>::endFinalStates( ) const
    {
      return states.endFinalStates();
    }  
    ...
    /*
    template<typename Client,typename Sym>
    const typename NWA<Client,Sym>::Symbols & NWA<Client,Sym>::getSymbols( )
    {
      return symbols;
    }*/
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::isSymbol( const Sym & sym ) const
    {
      return (symbols.count(sym) >  0);
    }    
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::addSymbol( const Sym & sym )
    {
      if( symbols.count(sym) > 0 )
        return false;
      symbols.insert(sym);
     ...
     */
    template<typename Client,typename Sym>
    void NWA<Client,Sym>::addAllSymbols( Symbols addSymbolSet )
    {
      for( symbolIterator sit = addSymbolSet.begin();
        sit != addSymbolSet.end(); sit++ )
        addSymbol(*sit);
     ...
     */
    template<typename Client,typename Sym>
    size_t NWA<Client,Sym>::sizeSymbols( ) const
    {
      return symbols.size();
    }
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::removeSymbol( const Sym & sym )
    {
     ...
     */
    template<typename Client,typename Sym>
    void NWA<Client,Sym>::clearSymbols( )
    {
      symbols.clear();
     ...
     */
    template<typename Client,typename Sym>
    typename NWA<Client,Sym>::symbolIterator NWA<Client,Sym>::beginSymbols( ) const
    {
      return symbols.begin();
    }
     ...
     */
    template<typename Client,typename Sym>
    typename NWA<Client,Sym>::symbolIterator NWA<Client,Sym>::endSymbols( ) const
    {
      return symbols.end();
    }
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::getSymbol( Key from, Key to, Sym & sym )
    {
      St * fromSt = getState(from);
      St * toSt = getState(to);
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::findTrans( Key from, const Sym & sym, Key to ) const
    {
      St* fromSt = getState(from);
      St* toSt = getState(to);
     ...
     */
    template<typename Client,typename Sym>
    void NWA<Client,Sym>::clearTrans( )
    {
      trans->clear();
    }    
    ...
    /* 
    template<typename Client,typename Sym>
    const typename NWA<Client,Sym>::Calls & NWA<Client,Sym>::getCallTrans( )
    {
      return trans->getCalls();
    }*/
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::addCallTrans( Key from, const Sym & sym, Key to )
    {
      St * fromSt = getState(from);
      St * toSt = getState(to);
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::addCallTrans( St * from, const Sym & sym, St * to )
    {
      if(! isState(from) )
        addState(from);
      if(! isState(to) )
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::addCallTrans( Call & ct )
    {
      if(! isState(&ct.first) )
        addState(&ct.first);
      if(! isState(&ct.third) )
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::removeCallTrans( Key from, const Sym & sym, Key to )
    {
      St * fromSt = getState(from);
      St * toSt = getState(to);
     ...
     */ 
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::removeCallTrans( St * from,const Sym & sym, St * to )
    {
      if(! isState(from)
        || ! isState(to) )
        return false;
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::removeCallTrans( const Call & ct )
    {
      if(! isState(&ct.first)
        || ! isState(&ct.third) )
        return false;
     ...
     */
    template<typename Client,typename Sym>
    typename NWA<Client,Sym>::callIterator NWA<Client,Sym>::beginCallTrans( ) const
    {
      return trans->beginCall();
    }
     ...
     */
    template<typename Client,typename Sym>
    typename NWA<Client,Sym>::callIterator NWA<Client,Sym>::endCallTrans( ) const
    {
      return trans->endCall();
    }
    ...
    /*
    template<typename Client,typename Sym>
    const typename NWA<Client,Sym>::Internals & NWA<Client,Sym>::getInternalTrans( )
    {
      return trans->getInternals();
    }*/
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::addInternalTrans( Key from,const Sym & sym, Key to )
    {
      St * fromSt = getState(from);
      St * toSt = getState(to);
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::addInternalTrans( St * from,const Sym & sym, St * to )
    {
      if(! isState(from) )
        addState(from);
      if(! isState(to) )
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::addInternalTrans( Internal & it )
    {
      if(! isState(&it.first) )
        addState(&it.first);
      if(! isState(&it.third) )
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::removeInternalTrans( Key from,const Sym & sym, Key to )
    {
      St * fromSt = getState(from);
      St * toSt = getState(to);
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::removeInternalTrans( St * from,const Sym & sym, St * to )
    {
      if(! isState(from)
        || ! isState(to) )
        return false;
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::removeInternalTrans(  const Internal & it )
    {
      if(! isState(&it.first)
        || ! isState(&it.third) )
        return false;
     ...
     */
    template<typename Client,typename Sym>
    typename NWA<Client,Sym>::internalIterator NWA<Client,Sym>::beginInternalTrans( ) const
    {
      return trans->beginInternal();
    }
     ...
     */
    template<typename Client,typename Sym>
    typename NWA<Client,Sym>::internalIterator NWA<Client,Sym>::endInternalTrans( ) const
    {
      return trans->endInternal();
    }
    ...
    /*
    template<typename Client,typename Sym>
    const typename NWA<Client,Sym>::Returns & NWA<Client,Sym>::getReturnTrans( )
    {
      return trans->getReturns();
    }*/
     ...
     */
    template<typename Client,typename Sym>
    std::set<Key> NWA<Client,Sym>::getReturnSites( Key callSite ) const
    {
      St * callSiteSt = getState(callSite);
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::addReturnTrans( Key from, Key pred,const Sym & sym, Key to )
    {
      St * fromSt = getState(from);
      St * predSt = getState(pred);
      St * toSt = getState(to);
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::addReturnTrans( St * from, St * pred, const Sym & sym, St * to )
    {
      if(! isState(from) )
        addState(from);
      if(! isState(pred) )
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::addReturnTrans( Return & rt )
    {
      if(! isState(&rt.first) )
        addState(&rt.first);
      if(! isState(&rt.second) )
        addState(&rt.second);
      if(! isState(&rt.fourth) )
        addState(&rt.fourth);
      if(! isSymbol(rt.third) )
        addSymbol(rt.third);  
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::removeReturnTrans( Key from,const Sym & sym, Key to )
    {
      St * fromSt = getState(from);
      St * toSt = getState(to);
      ...
      for( std::set<Key>::iterator it = calls.begin(); it != calls.end(); it++ )
      {
        St * predSt = getState(*it);
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::removeReturnTrans( St * from, const Sym & sym, St * to )
    {
      if(! isState(from)
        || ! isState(to) )
        return false;
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::removeReturnTrans( Key from, Key pred, const Sym & sym, Key to )
    {
      St * fromSt = getState(from);
      St * predSt = getState(pred);
      St * toSt = getState(to);
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::removeReturnTrans( St * from, St * pred, const Sym & sym, St * to )
    {
      if(! isState(from)
        || ! isState(pred)
        || ! isState(to) )
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::removeReturnTrans( const Return & rt )
    {
      if(! isState(&rt.first)
        || ! isState(&rt.second)
        || ! isState(&rt.fourth) )
     ...
     */
    template<typename Client,typename Sym>
    typename NWA<Client,Sym>::returnIterator NWA<Client,Sym>::beginReturnTrans( ) const
    {
      return trans->beginReturn();
    }
     ...
     */
    template<typename Client,typename Sym>
    typename NWA<Client,Sym>::returnIterator NWA<Client,Sym>::endReturnTrans( ) const
    {
      return trans->endReturn();
    }
    ...
    /*  Note: broken
    template<typename Client,typename Sym>
    NWA<Client,Sym> * NWA<Client,Sym>::unionNWA( NWA<Client,Sym> other )
    { //FIX
      //TODO: fix this for absentAcceptance and wild symbol
      NWA<Client,Sym> * newNWA = new NWA<Client,Sym>();
    ...
    /*  Note: broken
    template<typename Client,typename Sym>
    NWA<Client,Sym> * NWA<Client,Sym>::concat( NWA<Client,Sym> other )
    { //FIX
      //TODO: fix this for accept absent and wild symbol
      NWA<Client,Sym> * newNWA = new NWA<Client,Sym>();
    ...
    //TODO: check this with respect to calls/returns
    template<typename Client,typename Sym>
    NWA<Client,Sym> * NWA<Client,Sym>::star( )
    { //FIX
      //TODO: fix this for accept absent and wild symbol
      NWA<Client,Sym> * newNWA = new NWA<Client,Sym>();
    ...
    /* 
    template<typename Client,typename Sym>
    NWA<Client,Sym> * NWA<Client,Sym>::complement( )
    {
      NWA<Client,Sym> * newNWA = new NWA<Client,Sym>();
    ...
    /*  Note: broken
    template<typename Client,typename Sym>
    NWA<Client,Sym> & NWA<Client,Sym>::determinize( )
    { //FIX
      //TODO: how does this react to absentAcceptance & State/Symbol types & wild symbol
      std::pair<NWA<Client,Sym>,StateMap> tmp = std::pair<NWA<Client,Sym>,StateMap>();
    ...
    /* broken
    template<typename Client,typename Sym>
    std::pair<NWA<Client,Sym>,NWA<Client,Sym>::StateMap> NWA<Client,Sym><::det( NWA<Client,Sym> nWdAut, StateMap stateMap, StatePairSet currState,
                                                        std::stack<StatePairSet> callPred )
    { //FIX
      //TODO: absentAcceptance & State/Symbol type & wild symbol
      //Internal Transition
      ...
      {        
        ...
        if(recurse)
        {
          std::pair<NWA<Client,Sym>,StateMap> result = det(nWdAut,stateMap,tmpInternal,callPred);
          nWdAut = result.first;
          stateMap = result.second;
        }
      }    
      ...
      {        
        ...
        {
          callPred.push(currState);
          std::pair<NWA<Client,Sym>,StateMap> result = det(nWdAut,stateMap,tmpCall,callPred);
          nWdAut = result.first;
          stateMap = result.second;
        }
      }
      ...
      { 
        ...
        {
          callPred.pop();
          std::pair<NWA<Client,Sym>,StateMap> result = det(nWdAut,stateMap,tmpReturn,callPred);
          nWdAut = result.first;
          stateMap = result.second;
        }
      }
        
      return std::pair<NWA<Client,Sym>,StateMap>(nWdAut,stateMap);
    }*/
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::isDeterministic( )
    { 
      //TODO: optimize!!!
      //An NWA is not deterministic if there is not exactly one initial state
      if( sizeInitialStates() != 1) 
    ...
    /*  Note: broken
    template<typename Client,typename Sym>
    NWA<Client,Sym> * NWA<Client,Sym>::reverse()
    { //FIX
      //TODO: fix this for absent acceptance and wild symbol
      NWA<Client,Sym> * newNWA = new NWA<Client,Sym>();
    ...
    //TODO: incomplete...!!
    template<typename Client,typename Sym>
    void NWA<Client,Sym>::intersect( const NWA<Client,Sym> & other, NWA<Client,Sym> & result ) const
    {
      assert(!absentAcceptance && !other.absentAcceptance);
      //If both NWAs to be intersected accept absent, the intersection should too.
      //Otherwise, the intersection should not, so keep the default settings.
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::nodeIntersect( const St * node1, const St * node2, St * & result )
    {
      //Note: When overriding this method your metric must determine whether
      // the given states are compatible, then create a state and set result
      // to the state just created if they are compatible.
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::edgeIntersect( const Sym & label1, const Sym & label2, Sym & result )
    {
      //Note: When overriding this method your metric must determine an
      // appropriate label, create a symbol with that label, and set result
      // to the symbol just created.
    ...
    /*  Note: broken 
    template<typename Client,typename Sym>
    wpds::WPDS NWA<Client,Sym>::plusWPDS( wpds::WPDS base )
    { //FIX
      //TODO: how does this react to absentAcceptance & State/Symbol types & wild symbol
      wpds::WpdsRules rules = wpds::WpdsRules();
    ...
    /*  Note: broken
    template<typename Client,typename Sym>
    nwa::NWA<Client,Sym> NWA<Client,Sym>::PDStoNWA(wpds::WPDS pds)
    { //FIX
      //TODO: how does this react to absentAcceptance & State/Symbol types & wild symbol
      nwa::NWA<Client,Sym> result = nwa::NWA<Client,Sym>();
     ...
     */ 
    template<typename Client,typename Sym>
    wali::Key NWA<Client,Sym>::getProgramControlLocation()
    {
      return wali::getKey("program");
    } 
     ...
     */
    template<typename Client,typename Sym>
    wali::Key NWA<Client,Sym>::getControlLocation( const St & first, const St & second )
    {
      return getKey(first.getName(),second.getName());
    }  
     ...
     */
    template<typename Client,typename Sym>
    wpds::WPDS NWA<Client,Sym>::NWAtoPDS( WeightGen<St,Sym> & wg ) const
    { //FIX
      //TODO: how does this react to absentAcceptance???
      //NOTE: for now assume stuck state is rejecting
      wpds::WPDS result = wpds::WPDS();
      ...
      {      
        ...
        result.add_rule(program,                //from_state (p)
          (*iit).first.getName(),   //from_stack (q)
          program,                //to_state (p)
          (*iit).third.getName(),   //to_stack1 (q')
          wgt);                   //weight      
      }
        ...
        cit != trans->endCall(); cit++ )
      {
        for( returnIterator rit = trans->beginReturn();
          ...
          {
            //for each return site with cit->first as call site ...
            //Key ret = getKey(rit->fourth.getName(),rit->fourth.getName()); // (r,r)
            //calls.insert(std::pair<Key,Key>(cit->first.getName(),ret));
            calls.insert(std::pair<Key,Key>((*cit).first.getName(),(*rit).fourth.getName()));
            ...
            result.add_rule(program,                //from_state (p)
              (*cit).first.getName(),   //from_stack (q_c)
              program,                //to_state (p)
              (*cit).third.getName(),   //to_stack1 (q_e)
              //ret,                    //to_stack2 (r')
              (*rit).fourth.getName(),  //to_stack2 (r')
              wgt);                   //weight  
          }  
      }
        ...
        rit != trans->endReturn(); rit++ )
      {
        ...
        result.add_rule(program,                //from_state (p)
          (*rit).first.getName(),   //from_stack (q_x)
          rstate,                 //to_state (p_q_x == (q_x,q_c))
          wgt);                   //weight 
        ...
        std::map<Key,Key>::iterator ret = calls.find((*rit).second.getName());
        if( ret != calls.end() )          
          result.add_rule(rstate,                   //from_state (p_q_x == (q_x,q_c))
          ret->second,    //from_stack (r')
          program,                  //to_state (p)
          (*rit).fourth.getName(),    //to_stack (q_r)
          wgt);                     //weight    
     ...
     */
    template<typename Client,typename Sym>
    wpds::WPDS NWA<Client,Sym>::NWAtoBackwardsPDS( WeightGen<St,Sym> & wg ) const
    {
      //FIX
      //TODO: how does this react to absentAcceptance???
      //NOTE: for now assume stuck state is rejecting
      ...
      {      
        ...
        result.add_rule(program,                //from_state (p)
          (*iit).third.getName(),   //from_stack (q)
          program,                //to_state (p)
          (*iit).first.getName(),   //to_stack1 (q')
          wgt);                   //weight      
      }
        ...
        cit != trans->endCall(); cit++ )
      {
        for( returnIterator rit = trans->beginReturn();
          ...
          if( (*cit).first == (*rit).second )
          {
            ...
            result.add_rule(program,                //from_state (p)
              (*cit).third.getName(),   //from_stack (q_e)
              cstate,                 //to_state (p_q_e == (q_e,q_r))
              wgt);                   //weight 
            ...
            result.add_rule(cstate,                   //from_state (p_q_x == (q_x,q_c))
              (*cit).first.getName(),    //from_stack (q_c)
              program,                  //to_state (p)
              (*cit).first.getName(),    //to_stack (q_c)
              wgt);                     //weight    
          }  
      }
        ...
        rit != trans->endReturn(); rit++ )
      {
        ...
        result.add_rule(program,                //from_state (p)
          (*rit).fourth.getName(),   //from_stack (q_r)
          program,                //to_state (p)
          (*rit).first.getName(),   //to_stack1 (q_x)
          (*rit).second.getName(),  //to_stack2 (q_c)
          wgt);                   //weight      
      }
    ...
    /* broken
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::hasPath( St currState, std::stack< St > calls, std::set< St > visited )
    { //FIX
      //TODO: absentAcceptance and wild symbol
      //Try each possible transition out of this state to see if a final state
      //can be reached.
    ...
    /* broken
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::isMember( nws::NWS word )
    { //FIX
      //TODO: how does this react to absentAcceptance
      std::deque<WordRecConfig<St>> configs;
    ...
    /* broken
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::matchWord( St currState, std::deque<WordRecConfig<St>> configs )
    { //FIX
      //TODO: absentAcceptance and wild symbol
      WordRecConfig<St> config = configs.front();
      configs.pop_front();  //remove this config
     ...
     */
    template<typename Client,typename Sym>
    std::ostream & NWA<Client,Sym>::print( std::ostream & o ) const
    {
      states.print(o);
...
*/
    template<typename Client,typename Sym>
    std::ostream & NWA<Client,Sym>::print_dot( std::ostream & out) const
    {
      ...
      st->print(out << "\"") <<"\" [ style=bold ]";
      // final states
std::set<Key> finals = getFinalStates();
for(std::set<Key>::const_iterator it = finals.begin(); it!=finals.end(); it++) {
        St *st = getState( *it );
        st->print(out << "\"") <<"\" [ peripheries=2 ]";
      }
      trans->print_dot(out);
     ...
     */
    template<typename Client,typename Sym>
    bool NWA<Client,Sym>::operator==( const NWA<Client,Sym> & other ) const
    {
      return ( (states == other.states) &&
        (absentAcceptance == other.absentAcceptance) &&
        (symbols == other.symbols) &&   //TODO: deeper check here?
