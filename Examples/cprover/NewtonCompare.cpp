...
namespace goals {
  ...
  void endToEndCompare()
  {
    ...
    cout << "[Newton Compare] Goal II: end-to-end time comparison: FWPDS vs NWPDS" << endl;
    {
      ...
      cout << "[Newton Compare] Computing path summary..." << endl;
      outfa.path_summary(pds->get_theZero()->one());
      ...
      //DEBUGGING
      fstream dbg1("pds.dbg.dot", fstream::out);
      pds->print_dot(dbg1);
      fstream dbg2("outfa.dbg.dot", fstream::out);
      outfa.print_dot(dbg2);
      cout << "getPdsState() " << key2str(getPdsState()) << endl;
      cout << "getErrStk(pg) " << key2str(getErrStk(pg)) << endl;
      //
...
int main(int argc, char ** argv)
{
  ...
  originalPds = fwpds_from_prog(pg);
  if(dump){
