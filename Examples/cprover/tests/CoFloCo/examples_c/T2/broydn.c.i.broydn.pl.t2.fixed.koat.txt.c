...
int foo (int A, int B, int C, int D, int E, int F, int G, int H, int I, int J, int K, int L, int M, int N, int O, int P, int Q, int R, int S, int T, int U, int V, int W, int X, int Y, int Z, int A1, int B1, int C1) {
 ...
 __cost++;
   if (nondet_bool()) {
    ...
    int R_ = nondet();
    int U_ = 1;
    if (W_ >= Y_0 && V_ * Y_0 + Y_0 >= 1 + R_ && R_ >= V_ * Y_0 && Y_1 >= W_ && V_ * Y_1 + Y_1 >= 1 + R_ && R_ >= V_ * Y_1 && A >= B && V_ >= 2) {
     R = R_;
     S = S_;
     T = T_;
     U = U_;
 ...
 __cost++;
   ...
   if (nondet_bool()) {
    ...
    int Y_2 = nondet();
    int H_ = nondet();
    if (H_ >= Y_0 && Y_0 * Y_1 + Y_0 >= 1 + H && H >= Y_0 * Y_1 && Y_2 >= H_ && Y_1 * Y_2 + Y_2 >= 1 + H && H >= Y_1 * Y_2 && B >= 1 + A) {
     H = H_;
     goto loc_f58;
    }
   }
