...
echo "***************** Link phase: *****************"
g++ -g -o _build64/Examples/cprover/NewtonOcaml -Wl,-rpath=/unsup/boost-1.55.0/amd64_rhel6/lib -Wl,-rpath=$NEWTON_DUET_ROOT/_build/duet/src -Wl,-rpath=$WALI_ROOT -Wl,-rpath=$WALI_ROOT/lib64 -Wl,--start-group _build64/Examples/cprover/NewtonCompare.o -L/unsup/boost-1.55.0/amd64_rhel6/lib -Llib64 -L"`ocamlc -where`" -L$NEWTON_DUET_ROOT/_build/duet/src -L$WALI_ROOT -L$TSLRE_LIB_PATH -ltslre -lboost_filesystem -lboost_system -lboost_serialization -lrt -lduet -locamlinterface -lwali -lbpparser -lwalidomains -lglog -lbdd -Wl,--end-group || exit 1
