...
\begin{functionlist}
  \functionitemnonl[\texttt{appendInternal(Symbol s)}]
  \functionitemnonl[\texttt{appendCall(Symbol s)}]
  \functionitem[\texttt{appendReturn(Symbol s)}]
  ...
  \functionitem[\texttt{append(Position p)}]
    Appends \texttt{p} to the word.
  \functionitemnonl[\texttt{begin() const}]
  \functionitem[\texttt{end() const}]
