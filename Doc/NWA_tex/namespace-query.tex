...
\begin{functionlist}
  ...
  \functionDefFirst{bool}{languageIsEmpty}{NWA const \& nwa}{}
    Computes
    whether the NWA's language is empty, and returns the result. \TODO{This
      is the one place where I think we have to make a decision about how to
      support unbalanced words. Do we have one function
      that takes ``accept unbalanced left'' and ``accept unbalanced right''
      flags? Multiple (potentially four) functions? Set a state variable in
      the NWA which isEmpty queries? Say that you can always intersect your
      NWA with $(^n)^n$ if you want only balanced, so only provide a
