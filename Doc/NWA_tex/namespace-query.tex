...
Tabs.~\ref{Ta:query-all-transitions}--\ref{Ta:query-return-transitions} in \sectref{Tables}
...
\begin{functionlist}
  \functionitem[\texttt{bool isDeterministic(NWA const \& nwa)}]
    ...
    the library.
  \functionitem[\texttt{bool statesOverlap(NWA const \& a, NWA const \& b)}]
...
\begin{functionlist}
  \functionitem[\texttt{bool languageContains(NWA const \& nwa, Nested Word
  ...
  \functionitem[\texttt{bool languageIsEmpty(NWA const \& nwa)}] Computes
    whether the NWA's language is empty, and returns the result.
  \functionitem[\texttt{bool languageSubsetEq(NWA const \& left, NWA const \& right)}]
  ...
  \functionitem[\texttt{bool languageEquals(NWA const \& left, NWA const \& right)}]
...
\begin{functionlist}
  \functionitem[\texttt{WFA prestar( NWA const \& nwa, WFA const \& input, WeightGen \& wg )}]
  \functionitem[\texttt{void prestar( NWA const \& nwa, WFA const \& input, WFA \& output, WeightGen \& wg )}]
  ...
  \functionitem[\texttt{WFA poststar( NWA const \& nwa, WFA const \& input, WeightGen \& wg )}]
  \functionitem[\texttt{void poststar( NWA const \& nwa, WFA const \& input, WFA \& output, WeightGen \& wg )}]
