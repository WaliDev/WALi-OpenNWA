...
$w_i$.) The \textsc{Globally-Unbalanced} rule addresses the situation where
the current symbol is pending in the whole string. (This is the first case in
the previous paragraph.) The \textsc{Locally-Unbalanced} rule addresses the
situation where the current symbol is pending in the current $w_i$ but
matches a call from a previous $w_j$.
...
\and
{\inferrule*[left=\textsc{Globally-Unbalanced}]
  { (q,r,a,p) \in \delta_r \\ r \in Q_0 \\ s \in Q \cup Q' }
  {(q',s,a,p') \in  \delta_r^* }
}
\and
{\inferrule*[left=\textsc{Locally-Unbalanced}]
  { (q,r,a,p) \in \delta_r \\ r \in Q_0 \\ s \in Q \cup Q' }
  {(q',s,a,p') \in  \delta_r^* }
}
\end{mathpar}
