...
conditions. If the automaton is in a final state state of the input
...
\textsc{Start} rule allows $R$ to get from $q_0$ to the $A'$ portion.
transitions target $A'$ because there have been no calls read in the current
The \textsc{Globally-Pending} rule addresses the situation where the
...
\and
{\inferrule*[left=\textsc{Globally-Pending}]
  { (q,r,a,p) \in \delta_r \\ r \in Q_0 }
  {(q',q_0,a,p') \in  \delta_r^* }
}
\and
{\inferrule*[left=\textsc{Locally-Pending}]
  { (q,r,a,p) \in \delta_r \\ r \in Q_0 \\ s \in Q }
  {(q',s,a,p') \in \delta_r^* \\ (q', s', a, p') \in  \delta_r^* }
}
\end{mathpar}
