...
\begin{functionlist}
  ...
  \functionDef{void}{unionNWA}{NWA \& out, NWA const \& a, NWA const \& b}{}
    ...
    $O(|Q^a|+|Q^b|+|\delta^a|+|\delta^b|)$. \TODO{What do you think about
      putting these in here? I'm not 100\% sure they are all accurate, they
      aren't really counting ``lookup'' time (usually $\log |Q|$ or $\log
      |\delta|$, occasionally higher), in
      some cases they are only approximate (e.g. determinize). Should we
      leave them with a disclaimer? Remove them? Add more?}
  ...
  \functionDef{void}{concat}{NWA \& out, NWA const \& left, NWA const \& right}{}
    ...
    (The $\delta_r^b$ term can be refined to be the number of
  ...
  \functionDefFirstEarly{NWARefPtr}{complement}{NWA const \& orig, bool determinize}{}
  \functionDef{void}{complement}{NWA \& out, NWA const \& orig, bool determinize}{}
