...
A single $(p,q)$ pair means that the NWA was in state $p$, immediately read a
call symbol, and then read some substring of the input with balanced calls
and returns. This means that if the automaton then reads a return symbol, $q$
is the exit site and $p$ is the call predecessor: the two pieces of
information the automaton needs to decide what return transitions it can
take.
