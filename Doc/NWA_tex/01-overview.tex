...
transitions transitions (but not calls or returns). We also allow ``wild''
...
\begin{functionlist}
  \functionitem[\texttt{NestedWord}] Implements a single nested word. Currently the only
    ...
    \texttt{NestedWord} is a member of an \texttt{NWA}'s language. To use,
    include \texttt{wali/nwa/NestedWord.hpp}.
  \functionitem[\texttt{NWA}] Implements a nested-word automaton. To use, include
